...
def dismiss_marketing_message(request):
    if request.is_ajax():
        ...
        request.session['dismiss_message_for'] = str(timezone.localtime(timezone.now()) + datetime.timedelta(hours=settings.MARKETING_HOURS_OFFSET, seconds=settings.MARKETING_SECONDS_OFFSET))
        print(json_data)
